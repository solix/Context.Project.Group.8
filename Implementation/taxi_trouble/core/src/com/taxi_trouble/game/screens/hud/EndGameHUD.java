...
public class EndGameHUD implements HUDComponent {
	...
	/**
	 * Initializes an EndGameHUD displaying a message with winning team and
	 * their score on screen at position (xPos, yPos).
	 * 
	 * @param winner
	 *            : the winning team
	 * @param xPos
	 *            : x-position of the hud-component
	 * @param yPos
	 *            : y-position of the hud-component
	 */
	...
	public void render(SpriteBatch spriteBatch, BitmapFont hudFont) {
		spriteBatch.begin();
		String winnerText = String.format("Team %d won!",
				this.team.getTeamId() + 1);
		String winnerScore = String.format("Score: %d", this.team.getScore());
		hudFont.draw(spriteBatch, winnerText, xPosition, yPosition);
		hudFont.draw(spriteBatch, winnerScore, xPosition,
				yPosition - hudFont.getCapHeight());
		spriteBatch.end();
	}
}